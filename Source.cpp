...
public:
    ...
    virtual bool validareProgramare(bool asigurareSanatate, int varsta, int ora24, string* motiv) = 0;   //metoda virtual pura

    virtual void programare(string nume, bool asigurareSanatate, int varsta, int ora24) {           
        string motiv = " ";
        if (validareProgramare(asigurareSanatate, varsta, ora24, &motiv)) {
            cout << "Programare realizata pe numele " << nume << ". " << endl << endl;
        }
        else {
            cout << "Nu s-a putut realiza programarea pe numele " << nume << ". " << endl;
            cout << motiv << endl << endl;
        }
    }
};
...
public:
    //supraincarcare pointer
    virtual void metodaPura() {
...
class Spital:public Institutie {
protected:
    string nume;
    char* oras;
    string domeniu;
...
public:
    virtual bool validareProgramare(bool asigurareSanatate, int varsta, int ora24, string* motiv) {
        bool rezultat = true;
        if (ora24 >= 23 || ora24 < 6) {
            rezultat = false;
            *motiv = "Nu se pot face programari decat intre orele 6 - 23. ";
        }

        if (this->domeniu == "Public") {
            if (asigurareSanatate == false) {
                rezultat = false;
                *motiv = "Trebuie sa aveti asigurare de sanatate. ";
            }
        }
        return rezultat;
    };
...
public:
    virtual bool validareProgramare(bool asigurareSanatate, int varsta, int ora24, string* motiv) {
        ...
        if (varsta >= 18) {
            rezultat = false;
            *motiv = "Trebuie sa fiti persoana minora. ";
        }
        if (ora24 >= 23 || ora24 < 6) {
            rezultat = false;
            *motiv = "Nu se pot face programari decat intre orele 6 - 23. ";
...
void main() {
    int* v = new int[2] {3, 4};
    int* v1 = new int[4] {3, 4, 4, 5};

    Institutie** pointeriInstitutie;
    pointeriInstitutie = new Institutie * [10];
    pointeriInstitutie[0] = new Spital();
    pointeriInstitutie[1] = new SpitalPediatric();
    pointeriInstitutie[2] = new Spital("MediPlus", 2008, 150);
    pointeriInstitutie[3] = new SpitalPediatric("KidsHospital", "Brasov", "Privat", 2010, 40, 5, 2, v);
    pointeriInstitutie[4] = new Spital("Floreasca", "Bucuresti", "Public", 2003, 300);
    pointeriInstitutie[5] = new SpitalPediatric();
    pointeriInstitutie[6] = new Spital();
    pointeriInstitutie[7] = new Spital("Militar", "Cluj", "Public", 1999, 500);
    pointeriInstitutie[8] = new SpitalPediatric("Clinica Maria", "Targu Mures", "Public", 2005, 100, 10, 4, v1);
    pointeriInstitutie[9] = new Spital();

    pointeriInstitutie[0]->programare("Popescu Vlad", true, 40, 18);
    pointeriInstitutie[1]->programare("Mihai Tudor", true, 19, 18);
    pointeriInstitutie[2]->programare("Ana Matei", true, 19, 3);

    cout << "Programari: " << endl;
    for (int i = 3; i < 10; i++) {
        pointeriInstitutie[i]->programare("Mihailescu Laurentiu", true, 40, 18);
    }
